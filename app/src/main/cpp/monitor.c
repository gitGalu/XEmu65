...
const UBYTE MONITOR_optype6502[256] = {
        ,
...
static int parse_hex(const char *s, UWORD *hexval) {
    ...
    switch (s[0]) {
        case '\0':
            return FALSE; /* don't operate on empty string */
        case '*':
        case '@':
            deref_type = s[0];
            s++;
    ...
    for (p = mnemonic + 3; *p != '\0'; p++) {
        if (*p == '1') {
            ...
            nchars = fprintf(fp, "%04X: %02X %02X     " /*"%Xcyc  "*/ "%.*s$%02X%s",
                             addr, insn, value, /*cycles[insn],*/ (int) (p - mnemonic), mnemonic,
                             value, p + 1);
            break;
        }
        if (*p == '2') {
            ...
            nchars = fprintf(fp, "%04X: %02X %02X %02X  " /*"%Xcyc  "*/ "%.*s$%04X%s",
                             addr, insn, value & 0xff, value >> 8, /*cycles[insn],*/
                             (int) (p - mnemonic), mnemonic, value, p + 1);
            pc += 2;
            break;
        }
        if (*p == '0') {
            ...
            nchars = fprintf(fp, "%04X: %02X %02X     " /*"3cyc  "*/ "%.4s$%04X", addr, insn, op,
                             mnemonic, value);
            break;
        }
    }
    if (*p == '\0') {
        fprintf(fp, "%04X: %02X        " /*"%Xcyc  "*/ "%s\n", addr, insn, /*cycles[insn],*/
                mnemonic);
        return pc;
    }
#ifdef MONITOR_HINTS
...
static void show_state(void) {
    MONITOR_ShowState(stdout, CPU_regPC, CPU_regA, CPU_regX, CPU_regY, CPU_regS,
                      (char) ((CPU_regP & CPU_N_FLAG) ? 'N' : '-'),
                      (char) ((CPU_regP & CPU_V_FLAG) ? 'V' : '-'),
                      (char) ((CPU_regP & CPU_Z_FLAG) ? 'Z' : '-'),
                      (char) ((CPU_regP & CPU_C_FLAG) ? 'C' : '-'));
}
...
static void show_ANTIC(void) {
    printf("DMACTL=%02X    CHACTL=%02X    DLISTL=%02X    "
           ...
           ANTIC_DMACTL, ANTIC_CHACTL, ANTIC_dlist & 0xff, ANTIC_dlist >> 8, ANTIC_HSCROL,
           ANTIC_VSCROL);
