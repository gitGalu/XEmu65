...
void VOTRAXSND_PutByte(UBYTE byte) {
    ...
    votrax_sync_samples = (int) ((1.0 / ratio) *
                                 (double) Votrax_Samples((votrax_written_byte & 0x3f),
                                                         (byte & 0x3f), votrax_sync_samples));
    votrax_written = TRUE;
...
#ifdef VOICEBOX
    temp_votrax_buffer_size = (int) (VTRX_BLOCK_SIZE * ratio * (VOICEBOX_BASEAUDF + 1) +
                                     10); /* +10 .. little extra? */
#else
...
void VOTRAXSND_Frame(void) {
    ...
    if (VOICEBOX_enabled && VOICEBOX_ii) {
        ...
        samples_per_frame =
                ((double) dsprate / (double) (Atari800_tv_mode == Atari800_TV_PAL ? 50 : 60)) /
                factor;
    }
#endif
...
void VOTRAXSND_Process(void *sndbuffer, int sndn) {
    ...
    while (sndn > 0) {
        ...
        sndbuffer = (char *) sndbuffer +
                    VTRX_BLOCK_SIZE * (bit16 ? 2 : 1) * ((num_pokeys == 2) ? 2 : 1);
