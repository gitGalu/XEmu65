...
/* 0 .. Screen_WIDTH */
int Screen_visible_y2 = Screen_HEIGHT;
/* 0 .. Screen_HEIGHT */
...
void Screen_DrawAtariSpeed(double cur_time) {
    if (Screen_show_atari_speed) {
        ...
        if ((cur_time - last_time) >= 0.5) {
            percent_display = (int) (100 * (Atari800_nframes - last_updated) /
                                     (cur_time - last_time) /
                                     (Atari800_tv_mode == Atari800_TV_PAL ? 50 : 60));
    ...
    if (Screen_show_disk_led || Screen_show_sector_counter) {
        ...
        if (SIO_last_op_time > 0) {
            ...
            if (Screen_show_disk_led) {
                SmallFont_DrawChar(screen, SIO_last_drive, 0x00,
                                   (UBYTE) (SIO_last_op == SIO_LAST_READ ? 0xac : 0x2b));
                SmallFont_DrawChar(screen -= SMALLFONT_WIDTH, SMALLFONT_D, 0x00,
                                   (UBYTE) (SIO_last_op == SIO_LAST_READ ? 0xac : 0x2b));
                screen -= SMALLFONT_WIDTH;
            }
        ...
        if ((CASSETTE_readable && !CASSETTE_record) ||
            ...
            if (Screen_show_disk_led)
                SmallFont_DrawChar(screen, SMALLFONT_C, 0x00,
                                   (UBYTE) (CASSETTE_record ? 0x2b : 0xac));
            ...
            if (Screen_show_sector_counter) {
                ...
                if (!CASSETTE_record) {
                    screen = SmallFont_DrawInt(screen - SMALLFONT_WIDTH, CASSETTE_GetSize(), 0x00,
                                               0x88);
...
static void Screen_SavePCX(FILE *fp, UBYTE *ptr1, UBYTE *ptr2) {
    ...
    for (y = 0; y < Screen_HEIGHT;) {
        x = 0;
        do {
            last = ptr2 != NULL ? (((Colours_table[*ptr1] >> plane) & 0xff) +
                                   ((Colours_table[*ptr2] >> plane) & 0xff)) >> 1 : *ptr1;
            count = 0xc0;
            do {
                ptr1++;
                if (ptr2 != NULL)
                ...
                count++;
                x++;
            } while (last == (ptr2 != NULL ? (((Colours_table[*ptr1] >> plane) & 0xff) +
                                              ((Colours_table[*ptr2] >> plane) & 0xff)) >> 1
                                           : *ptr1)
