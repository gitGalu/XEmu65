...
static int cassette_gapdelay = 0;
/* in ms, includes leader and all gaps */
...
void CASSETTE_PutByte(int byte) {
    if (!ESC_enable_sio_patch && CASSETTE_writable && CASSETTE_record)
        IMG_TAPE_WriteByte(cassette_file, byte,
                           POKEY_AUDF[POKEY_CHAN3] + POKEY_AUDF[POKEY_CHAN4] * 0x100);
}
...
int CASSETTE_ToggleRecord(void) {
    ...
    return !CASSETTE_record ||
           (CASSETTE_status == CASSETTE_STATUS_READ_WRITE && !CASSETTE_write_protect);
}
