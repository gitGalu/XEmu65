...
int MEMORY_axlon_0f_mirror = FALSE;
...
void MEMORY_StateRead(UBYTE SaveVerbose, UBYTE StateVersion) {
    ...
    if (MEMORY_ram_size > 64) {
        ...
        if (StateVersion >= 7 &&
            (MEMORY_ram_size == 128 || MEMORY_ram_size == MEMORY_RAM_320_COMPY_SHOP)) {
            switch (portb & 0x30) {
    ...
    /* Simius XL/XE MapRAM expansion */
    if (StateVersion >= 7 && Atari800_machine_type == Atari800_MACHINE_XLXE &&
        MEMORY_ram_size > 20) {
    ...
    if (MEMORY_ram_size > 64) {
        ...
        if (MEMORY_selftest_enabled
            ...
            if (ANTIC_xe_ptr != NULL)
                ...
                memcpy(atarixe_memory + (antic_bank << 14) + 0x1000, antic_bank_under_selftest,
                       0x800);
        ...
        if (MEMORY_ram_size == 128 || MEMORY_ram_size == MEMORY_RAM_320_COMPY_SHOP)
            ANTIC_xe_ptr =
                    new_antic_bank == new_cpu_bank ? NULL : atarixe_memory + (new_antic_bank << 14);
        ...
        if (byte & 0x01) {
            ...
            if (MEMORY_selftest_enabled) {
                if (MEMORY_ram_size > 20) {
                    ...
                    if (ANTIC_xe_ptr != NULL)
                        /* Also disable Self Test from XE bank accessed by ANTIC. */
                        memcpy(atarixe_memory + (antic_bank << 14) + 0x1000,
                               antic_bank_under_selftest, 0x800);
                    MEMORY_SetRAM(0x5000, 0x57ff);
                }
                else
        ...
        if (!MEMORY_selftest_enabled && (byte & 0x01)
            ...
            if (MEMORY_ram_size > 20) {
                ...
                if (ANTIC_xe_ptr != NULL)
                    ...
                    memcpy(antic_bank_under_selftest, atarixe_memory + (antic_bank << 14) + 0x1000,
                           0x800);
...
#endif
    newbank = addr - 0xffc0;
    if (newbank == mosaic_curbank ||
        (newbank >= mosaic_current_num_banks && mosaic_curbank >= mosaic_current_num_banks))
        return; /*same bank or rom -> rom*/
