...
static void Devices_H_Delete(void) {
...
#endif /* DO_DIR */
    {
        ...
        if (readonly)
            num_locked++;
        else if (Util_unlink(host_path) == 0)
            num_deleted++;
        else
            num_failed++;
    }
...
static void Devices_H_Point(void) {
    ...
    if (h_fp[h_iocb] != NULL) {
        ...
        long pos = (MEMORY_dGetByte(iocb + Devices_ICAX4) << 16) +
                   (MEMORY_dGetByte(iocb + Devices_ICAX3) << 8) +
                   (MEMORY_dGetByte(iocb + Devices_ICAX5));
...
static void Devices_P_Close(void) {
    ...
    if (phf != NULL) {
        ...
        {
            char command[256 +
                         FILENAME_MAX]; /* 256 for Devices_print_command + FILENAME_MAX for spool_file */
            int retval;
