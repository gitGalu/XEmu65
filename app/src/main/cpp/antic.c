...
static UBYTE anticmode;
/* Antic mode */
static UBYTE dctr;
/* Delta Counter */
static UBYTE lastline;
/* dctr limit */
static UBYTE need_dl;
/* boolean: fetch DL next line */
...
static void pmg_dma(void) {
    ...
    if (ANTIC_missile_dma_enabled) {
        if (ANTIC_missile_gra_enabled) {
            UBYTE data;
            if (ANTIC_xe_ptr != NULL && pmbase_s < 0x8000 && pmbase_s >= 0x4000)
                data = ANTIC_xe_ptr[singleline ? pmbase_s + ANTIC_ypos + 0x300 - 0x4000 : pmbase_d +
                                                                                          (ANTIC_ypos >>
                                                                                           1) +
                                                                                          0x180 -
                                                                                          0x4000];
            else
                data = MEMORY_dGetByte(
                        singleline ? pmbase_s + ANTIC_ypos + 0x300 : pmbase_d + (ANTIC_ypos >> 1) +
                                                                     0x180);
            /* in odd lines load all missiles, in even only those, for which VDELAY bit is zero */
            GTIA_GRAFM = ANTIC_ypos & 1 ? data :
                         ((GTIA_GRAFM ^ data) & hold_missiles_tab[GTIA_VDELAY & 0xf]) ^ data;
        }
        ANTIC_xpos++;
    }
}
...
static void do_border(void) {
    ...
    /* left border */
    for (kk = left_border_chars; kk; kk--) DO_BORDER
    /* right border */
    ptr = &scrn_ptr[right_border_start];
    pm_scanline_ptr = &GTIA_pm_scanline[right_border_start];
    while (pm_scanline_ptr < &GTIA_pm_scanline[RBORDER_END]) DO_BORDER
}
...
static void do_border_gtia10(void) {
    ...
    /* left border */
    for (kk = left_border_chars; kk; kk--) DO_GTIA10_BORDER
    WRITE_VIDEO(ptr, COLOUR(pm_lookup_ptr[*pm_scanline_ptr |
                                          1])); /* one extra pixel, because of the right shift of gtia10*/
    /* right border */
    pm_scanline_ptr = &GTIA_pm_scanline[right_border_start];
    if (pm_scanline_ptr < &GTIA_pm_scanline[RBORDER_END]) {
        ptr = &scrn_ptr[right_border_start +
                        1]; /*start one pixel further right because of the right shift of gtia10*/
        ...
        pm_scanline_ptr += 4;
        while (pm_scanline_ptr < &GTIA_pm_scanline[RBORDER_END]) DO_GTIA10_BORDER
    }
}
...
#else
                WRITE_VIDEO(ptr, pixel ? (pixel << 4) | (pixel << 12) | ANTIC_cl[C_PF3] :
                                 ANTIC_cl[C_PF3] & 0xf0f0);
#endif
            }
            else {
...
#else
                WRITE_VIDEO(ptr + 1, pixel ? (pixel << 4) | (pixel << 12) | ANTIC_cl[C_PF3] :
                                     ANTIC_cl[C_PF3] & 0xf0f0);
#endif
            }
            else {
...
static void draw_antic_2(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                         const ULONG *t_pm_scanline_ptr) {
    INIT_BACKGROUND_6
    INIT_ANTIC_2
    INIT_HIRES
    ...
    CHAR_LOOP_BEGIN
        ...
        if (IS_ZERO_ULONG(t_pm_scanline_ptr)) {
            ...
            else DRAW_BACKGROUND(C_PF2)
        }
        else DO_PMG_HIRES(chdata)
        t_pm_scanline_ptr++;
            CHAR_LOOP_END
    do_border();
}
...
static void draw_antic_2_dmactl_bug(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                                    const ULONG *t_pm_scanline_ptr) {
    INIT_BACKGROUND_6
    INIT_ANTIC_2
...
static void draw_antic_2_artif(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                               const ULONG *t_pm_scanline_ptr) {
    ...
    CHAR_LOOP_BEGIN
        ...
        screendata_tally <<= 8;
        screendata_tally |= chdata;
        if (IS_ZERO_ULONG(t_pm_scanline_ptr)) DRAW_ARTIF
        else {
...
static void draw_antic_2_artif_new(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                                   const ULONG *t_pm_scanline_ptr) {
    ULONG screendata_tally;
    ULONG pmtally;
    UBYTE screendata = *antic_memptr++;
    ...
    CHAR_LOOP_BEGIN
        ...
        screendata_tally <<= 8;
        screendata_tally |= chdata;
        if (IS_ZERO_ULONG(t_pm_scanline_ptr)) DRAW_ARTIF_NEW
        else {
            chdata = screendata_tally >> 8;
...
static void prepare_an_antic_2(int nchars, const UBYTE *antic_memptr,
                               const ULONG *t_pm_scanline_ptr) {
...
static void draw_antic_2_gtia9(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                               const ULONG *t_pm_scanline_ptr) {
    INIT_ANTIC_2
...
static void draw_antic_2_gtia10(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                                const ULONG *t_pm_scanline_ptr) {
#ifdef WORDS_UNALIGNED_OK
...
static void draw_antic_2_gtia11(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                                const ULONG *t_pm_scanline_ptr) {
    INIT_ANTIC_2
    if ((unsigned long) ptr & 2) { /* HSCROL & 1 */
        prepare_an_antic_2(nchars, antic_memptr, t_pm_scanline_ptr);
...
#else
                        WRITE_VIDEO(ptr, tmp ? tmp | ((UWORD) tmp << 8) | ANTIC_cl[C_PF3] :
                                         ANTIC_cl[C_PF3] & 0xf0f0);
#endif
                    }
                    else {
...
static void draw_antic_2_gtia_bug(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                                  const ULONG *t_pm_scanline_ptr) {
    prepare_an_antic_2(nchars, antic_memptr, t_pm_scanline_ptr);
...
static void draw_antic_4(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                         const ULONG *t_pm_scanline_ptr) {
    INIT_BACKGROUND_8
#ifdef PAGED_MEM
...
static void prepare_an_antic_4(int nchars, const UBYTE *antic_memptr,
                               const ULONG *t_pm_scanline_ptr) {
    UBYTE *an_ptr = (UBYTE *) t_pm_scanline_ptr + (an_scanline - GTIA_pm_scanline);
#ifdef PAGED_MEM
    UWORD t_chbase = ((anticmode == 4 ? dctr : dctr >> 1) ^ chbase_20) & 0xfc07;
...
static void draw_antic_6(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                         const ULONG *t_pm_scanline_ptr) {
#ifdef PAGED_MEM
...
static void prepare_an_antic_6(int nchars, const UBYTE *antic_memptr,
                               const ULONG *t_pm_scanline_ptr) {
    UBYTE *an_ptr = (UBYTE *) t_pm_scanline_ptr + (an_scanline - GTIA_pm_scanline);
#ifdef PAGED_MEM
    UWORD t_chbase = (anticmode == 6 ? dctr & 7 : dctr >> 1) ^ chbase_20;
...
static void draw_antic_8(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                         const ULONG *t_pm_scanline_ptr) {
...
static void prepare_an_antic_8(int nchars, const UBYTE *antic_memptr,
                               const ULONG *t_pm_scanline_ptr) {
    UBYTE *an_ptr = (UBYTE *) t_pm_scanline_ptr + (an_scanline - GTIA_pm_scanline);
    CHAR_LOOP_BEGIN
        UBYTE screendata = *antic_memptr++;
...
static void draw_antic_9(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                         const ULONG *t_pm_scanline_ptr) {
    lookup2[0x00] = ANTIC_cl[C_BAK];
...
static void draw_antic_9_gtia9(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                               const ULONG *t_pm_scanline_ptr) {
    draw_antic_0();
}

static void draw_antic_9_gtia10(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                                const ULONG *t_pm_scanline_ptr) {
    draw_antic_0_gtia10();
}

static void draw_antic_9_gtia11(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                                const ULONG *t_pm_scanline_ptr) {
    draw_antic_0_gtia11();
}

static void draw_antic_a(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                         const ULONG *t_pm_scanline_ptr) {
    lookup2[0x00] = ANTIC_cl[C_BAK];
...
static void prepare_an_antic_a(int nchars, const UBYTE *antic_memptr,
                               const ULONG *t_pm_scanline_ptr) {
    UBYTE *an_ptr = (UBYTE *) t_pm_scanline_ptr + (an_scanline - GTIA_pm_scanline);
    CHAR_LOOP_BEGIN
        UBYTE screendata = *antic_memptr++;
...
static void draw_antic_c(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                         const ULONG *t_pm_scanline_ptr) {
    lookup2[0x00] = ANTIC_cl[C_BAK];
...
static void draw_antic_e(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                         const ULONG *t_pm_scanline_ptr) {
    INIT_BACKGROUND_8
    lookup2[0x00] = ANTIC_cl[C_BAK];
    lookup2[0x40] = lookup2[0x10] = lookup2[0x04] = lookup2[0x01] = ANTIC_cl[C_PF0];
...
static void prepare_an_antic_e(int nchars, const UBYTE *antic_memptr,
                               const ULONG *t_pm_scanline_ptr) {
    UBYTE *an_ptr = (UBYTE *) t_pm_scanline_ptr + (an_scanline - GTIA_pm_scanline);
    CHAR_LOOP_BEGIN
        UBYTE screendata = *antic_memptr++;
...
static void draw_antic_e_gtia9(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                               const ULONG *t_pm_scanline_ptr) {
    ULONG lookup[16];
    if ((unsigned long) ptr & 2) { /* HSCROL & 1 */
...
static void draw_antic_e_gtia10(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                                const ULONG *t_pm_scanline_ptr) {
    prepare_an_antic_e(nchars, antic_memptr, t_pm_scanline_ptr);
    draw_an_gtia10(t_pm_scanline_ptr);
}

static void draw_antic_e_gtia11(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                                const ULONG *t_pm_scanline_ptr) {
    prepare_an_antic_e(nchars, antic_memptr, t_pm_scanline_ptr);
    draw_an_gtia11(t_pm_scanline_ptr);
}

static void draw_antic_f(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                         const ULONG *t_pm_scanline_ptr) {
    INIT_BACKGROUND_6
    INIT_HIRES
    ...
    CHAR_LOOP_BEGIN
        ...
        else DO_PMG_HIRES(screendata)
        t_pm_scanline_ptr++;
            CHAR_LOOP_END
    do_border();
}

static void draw_antic_f_artif(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                               const ULONG *t_pm_scanline_ptr) {
...
static void draw_antic_f_artif_new(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                                   const ULONG *t_pm_scanline_ptr) {
    ULONG pmtally;
    ULONG screendata_tally = *antic_memptr++;
    INIT_ARTIF_NEW
...
static void prepare_an_antic_f(int nchars, const UBYTE *antic_memptr,
                               const ULONG *t_pm_scanline_ptr) {
    UBYTE *an_ptr = (UBYTE *) t_pm_scanline_ptr + (an_scanline - GTIA_pm_scanline);
    CHAR_LOOP_BEGIN
        UBYTE screendata = *antic_memptr++;
...
static void draw_antic_f_gtia9(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                               const ULONG *t_pm_scanline_ptr) {
    if ((unsigned long) ptr & 2) { /* HSCROL & 1 */
...
static void draw_antic_f_gtia10(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                                const ULONG *t_pm_scanline_ptr) {
#ifdef WORDS_UNALIGNED_OK
    ULONG lookup_gtia10[16];
#else
    ...
    CHAR_LOOP_BEGIN
        ...
        if (IS_ZERO_ULONG(t_pm_scanline_ptr)) {
            ...
            do {
                colreg = gtia_10_lookup[t_screendata];
                PF_COLLS(
                        colreg) |= pm_pixel = *c_pm_scanline_ptr++; /*playfield colours can generate collisions*/
                pm_pixel |= gtia_10_pm[t_screendata]; /*but player colours don't*/
                WRITE_VIDEO(ptr++, COLOUR(pm_lookup_ptr[pm_pixel] |
                                          colreg)); /*although they mix with the real players*/
                if (k == 3)
...
static void draw_antic_f_gtia11(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                                const ULONG *t_pm_scanline_ptr) {
    if ((unsigned long) ptr & 2) { /* HSCROL & 1 */
        prepare_an_antic_f(nchars, antic_memptr, t_pm_scanline_ptr);
        draw_an_gtia11(t_pm_scanline_ptr);
...
static void draw_antic_f_gtia_bug(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                                  const ULONG *t_pm_scanline_ptr) {
...
typedef void (*draw_antic_function)(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                                    const ULONG *t_pm_scanline_ptr);
...
static void draw_antic_0_dmactl_bug(int nchars, const UBYTE *antic_memptr, UWORD *ptr,
                                    const ULONG *t_pm_scanline_ptr) {
    draw_antic_0_ptr();
}
...
#define ART_GREEN 7
    ...
    art_colours = (ANTIC_artif_mode <= 4 ? art_colour_table[ANTIC_artif_mode - 1]
                                         : art_colour_table[2]);
    ...
    if ((screenaddr ^ new_screenaddr) & 0xf000) {
        ...
        if (ANTIC_xe_ptr != NULL && screenaddr < 0x8000 && screenaddr >= 0x4000) {
            ...
            if (new_screenaddr & 0xfff)
                memcpy(antic_memory + ANTIC_margin + bytes,
                       ANTIC_xe_ptr + (screenaddr + bytes - 0x5000), new_screenaddr & 0xfff);
        }
        else if ((screenaddr & 0xf000) == 0xd000) {
            MEMORY_CopyFromMem(screenaddr, antic_memory + ANTIC_margin, bytes);
            if (new_screenaddr & 0xfff)
                MEMORY_CopyFromMem((UWORD) (screenaddr + bytes - 0x1000),
                                   antic_memory + ANTIC_margin + bytes, new_screenaddr & 0xfff);
        }
        else {
            MEMORY_dCopyFromMem(screenaddr, antic_memory + ANTIC_margin, bytes);
            if (new_screenaddr & 0xfff)
                MEMORY_dCopyFromMem(screenaddr + bytes - 0x1000,
                                    antic_memory + ANTIC_margin + bytes, new_screenaddr & 0xfff);
        }
        ...
        if (ANTIC_xe_ptr != NULL && screenaddr < 0x8000 && screenaddr >= 0x4000)
            memcpy(antic_memory + ANTIC_margin, ANTIC_xe_ptr + (screenaddr - 0x4000),
                   chars_read[md]);
        else if ((screenaddr & 0xf000) == 0xd000)
...
void ANTIC_Frame(int draw_display) {
    ...
    need_dl = TRUE;
    do {
        if ((INPUT_mouse_mode == INPUT_MOUSE_PEN || INPUT_mouse_mode == INPUT_MOUSE_GUN) &&
            (ANTIC_ypos >> 1 == ANTIC_PENV_input)) {
        ...
        if (need_dl) {
            if (ANTIC_DMACTL & 0x20) {
                ...
                if (ANTIC_missile_flickering)
                    GTIA_GRAFM = ANTIC_ypos & 1 ? IR :
                                 ((GTIA_GRAFM ^ IR) & hold_missiles_tab[GTIA_VDELAY & 0xf]) ^ IR;
...
static void set_dmactl_bug(void) {
    ...
    if (anticmode == 2 || anticmode == 3 || anticmode == 0xf) {
        ...
        dmactl_bug_chdata = (anticmode == 0xf) ? 0 : antic_memory[ANTIC_margin + chars_read[md] -
                                                                  1];
    }
    else {
...
void ANTIC_SetPrior(UBYTE byte) {
    if ((byte ^ GTIA_PRIOR) & 0x0f) {
        ...
        if (byte & 4) {
            ANTIC_cl[C_PF2 | C_PM01] = ANTIC_cl[C_PF2 | C_PM1] = ANTIC_cl[C_PF2 |
                                                                          C_PM0] = ANTIC_cl[C_PF2];
            ANTIC_cl[C_PF3 | C_PM01] = ANTIC_cl[C_PF3 | C_PM1] = ANTIC_cl[C_PF3 |
                                                                          C_PM0] = ANTIC_cl[C_PF3];
        }
        else {
            ANTIC_cl[C_PF3 | C_PM0] = ANTIC_cl[C_PF2 | C_PM0] = ANTIC_cl[C_PM0];
            ANTIC_cl[C_PF3 | C_PM1] = ANTIC_cl[C_PF2 | C_PM1] = ANTIC_cl[C_PM1];
        ...
        if (byte & 1) {
            ...
            ANTIC_cl[C_PF0 | C_PM23] = ANTIC_cl[C_PF0 | C_PM3] = ANTIC_cl[C_PF0 |
                                                                          C_PM2] = ANTIC_cl[C_PF0];
            ANTIC_cl[C_PF1 | C_PM23] = ANTIC_cl[C_PF1 | C_PM3] = ANTIC_cl[C_PF1 |
                                                                          C_PM2] = ANTIC_cl[C_PF1];
        }
        if ((byte & 0xf) == 0xc) {
            ANTIC_cl[C_PF0 | C_PM0123] = ANTIC_cl[C_PF0 | C_PM123] = ANTIC_cl[C_PF0 |
                                                                              C_PM023] = ANTIC_cl[C_PF0];
            ANTIC_cl[C_PF1 | C_PM0123] = ANTIC_cl[C_PF1 | C_PM123] = ANTIC_cl[C_PF1 |
                                                                              C_PM023] = ANTIC_cl[C_PF1];
        }
        else
            ANTIC_cl[C_PF0 | C_PM0123] = ANTIC_cl[C_PF0 | C_PM123] = ANTIC_cl[C_PF0 | C_PM023] =
            ANTIC_cl[C_PF1 | C_PM0123] = ANTIC_cl[C_PF1 | C_PM123] = ANTIC_cl[C_PF1 |
                                                                              C_PM023] = GTIA_COLOUR_BLACK;
        if (byte & 0xf) {
            ANTIC_cl[C_PF0 | C_PM25] = ANTIC_cl[C_PF0];
            ANTIC_cl[C_PF1 | C_PM25] = ANTIC_cl[C_PF1];
            ANTIC_cl[C_PF3 | C_PM25] = ANTIC_cl[C_PF2 |
                                                C_PM25] = ANTIC_cl[C_PM25] = GTIA_COLOUR_BLACK;
        }
        else {
            ANTIC_cl[C_PF0 | C_PM235] = ANTIC_cl[C_PF0 | C_PM35] = ANTIC_cl[C_PF0 | C_PM25] =
            ANTIC_cl[C_PF1 | C_PM235] = ANTIC_cl[C_PF1 | C_PM35] = ANTIC_cl[C_PF1 |
                                                                            C_PM25] = ANTIC_cl[C_PF3];
            ANTIC_cl[C_PF3 | C_PM25] = ANTIC_cl[C_PF2 | C_PM25] = ANTIC_cl[C_PM25] = ANTIC_cl[
                    C_PF3 | C_PM2];
            ANTIC_cl[C_PF3 | C_PM35] = ANTIC_cl[C_PF2 | C_PM35] = ANTIC_cl[C_PM35] = ANTIC_cl[
                    C_PF3 | C_PM3];
            ANTIC_cl[C_PF3 | C_PM235] = ANTIC_cl[C_PF2 | C_PM235] = ANTIC_cl[C_PM235] = ANTIC_cl[
                    C_PF3 | C_PM23];
        }
    }
    pm_lookup_ptr = pm_lookup_table[prior_to_pm_lookup[byte & 0x3f]];
    draw_antic_0_ptr =
            byte < 0x80 ? draw_antic_0 : byte < 0xc0 ? draw_antic_0_gtia10 : draw_antic_0_gtia11;
    if (byte < 0x40 && (GTIA_PRIOR >= 0x40 || gtia_bug_active) &&
        (anticmode == 2 || anticmode == 3 || anticmode == 0xf) &&
        ANTIC_XPOS >= ((ANTIC_DMACTL & 3) == 3 ? 16 : 18)) {
...
glitch */
            ...
            if ((byte & 3) != (ANTIC_DMACTL & 3)) {
                ...
                if ((ANTIC_DMACTL & 3) == 0) {
                    ...
                    if (((byte & 3) > (ANTIC_DMACTL & 3)) && !(IR & 0x10)) {
                        ...
                        else if (((ANTIC_DMACTL & 3) == 1) &&
                                 ((byte & 3) == 3)) { /* Narrow->Wide */
                            left_glitch_cycle = 11;
                            right_glitch_cycle = 26;
                        }
                        else if (((ANTIC_DMACTL & 3) == 1) &&
                                 ((byte & 3) == 2)) { /* Narrow->Normal */
                        ...
                        else if (x >= left_glitch_cycle && x <= right_glitch_cycle &&
                                 anticmode > 1) {
                            set_dmactl_bug();
                        }
                    }
            ...
            ANTIC_player_flickering = ((ANTIC_player_dma_enabled | ANTIC_player_gra_enabled) ==
                                       0x02);
            ANTIC_missile_flickering = ((ANTIC_missile_dma_enabled | ANTIC_missile_gra_enabled) ==
                                        0x01);
...
#ifdef NEW_CYCLE_EXACT
            if (ANTIC_DRAWING_SCREEN) {
                if (ANTIC_xpos <= ANTIC_antic2cpu_ptr[ANTIC_WSYNC_C] &&
                    ANTIC_xpos_limit >= ANTIC_antic2cpu_ptr[ANTIC_WSYNC_C]) if (
                        ANTIC_cpu2antic_ptr[ANTIC_xpos + 1] ==
                        ANTIC_cpu2antic_ptr[ANTIC_xpos] + 1) {
...
case we have ANTIC_cpu2antic_ptr[ANTIC_WSYNC_C+1]-1 = 8 and in the 2nd =12  */
                    ...
                    if (ANTIC_cpu2antic_ptr[ANTIC_xpos + 1] ==
                        ANTIC_cpu2antic_ptr[ANTIC_xpos] + 1) {
                        /* antic does not steal the current cycle */
