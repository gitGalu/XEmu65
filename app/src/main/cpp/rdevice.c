...
void RDevice_WRIT(void) {
    ...
    if (translation) {
        if (CPU_regA == 0x9b) {
            out_char = 0x0d;
            if (linefeeds) {
                if ((RDevice_serial_enabled == 0) && (connected == 0)) { /* local echo */
                    bufend++;
...
#endif /* R_NETWORK */
#ifndef DREAMCAST
    if ((connected) && (write(rdev_fd, (char *) &out_char, 1) <
                        1)) { /* returns -1 if disconnected or 0 if could not send */
        perror("write");
            ...
            if (bytesread > 0) {
                if ((RDevice_serial_enabled == 0) && (one == 0xff)) {
                    ...
                    if (telnet_command[0] == 0xfd) { /*DO*/
                        if ((telnet_command[1] == 0x01) ||
                            (telnet_command[1] == 0x03)) { /* WILL ECHO and GO AHEAD (char mode) */
