...
void PAL_BLENDING_UpdateLookup(void) {
    if (ARTIFACT_mode == ARTIFACT_PAL_BLEND) {
        ...
        shift_mask = (format.rmask & ~(format.rmask << 1)) | (format.gmask & ~(format.gmask << 1)) |
...
void PAL_BLENDING_BlitScaled16(ULONG *dest, UBYTE *src, int pitch, int width, int height,
                               int dest_width, int dest_height, int start_odd) {
    register ULONG quad, quad_prev;
    register int x;
    int y = 0x10000;
...
void PAL_BLENDING_BlitScaled32(ULONG *dest, UBYTE *src, int pitch, int width, int height,
                               int dest_width, int dest_height, int start_odd) {
    register ULONG quad, quad_prev;
    register int x;
    int y = 0x10000;
