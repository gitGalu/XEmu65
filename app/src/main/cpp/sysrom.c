...
static int MatchByName(char const *filename, int len, int only_if_not_set) {
    static char const *const common_filenames[] = {
            ...
            NULL,
            "ataribasic.rom", "ataribas.rom", "basic.rom", "atari_basic.rom", "REVA.ROM",
            "REVB.ROM", "REVC.ROM",
            NULL
    };
    static struct {
...
static int const autochoose_order_800_ntsc[] = {SYSROM_B_NTSC, SYSROM_A_NTSC, SYSROM_A_PAL,
                                                SYSROM_800_CUSTOM, -1};
static int const autochoose_order_800_pal[] = {SYSROM_A_PAL, SYSROM_B_NTSC, SYSROM_A_NTSC,
                                               SYSROM_800_CUSTOM, -1};
static int const autochoose_order_1200xl[] = {SYSROM_AA01R11, SYSROM_AA00R10, SYSROM_BB00R1,
                                              SYSROM_BB01R2, SYSROM_BB01R3, SYSROM_BB01R4_OS,
                                              SYSROM_BB01R59, SYSROM_BB01R59A, SYSROM_BB02R3,
                                              SYSROM_BB02R3V4, SYSROM_CC01R4, SYSROM_XL_CUSTOM, -1};
static int const autochoose_order_600xl[] = {SYSROM_BB00R1, SYSROM_BB01R2, SYSROM_BB01R3,
                                             SYSROM_BB01R4_OS, SYSROM_AA01R11, SYSROM_AA00R10,
                                             SYSROM_BB01R59, SYSROM_BB01R59A, SYSROM_BB02R3,
                                             SYSROM_BB02R3V4, SYSROM_CC01R4, SYSROM_XL_CUSTOM, -1};
static int const autochoose_order_800xl[] = {SYSROM_BB01R2, SYSROM_BB01R3, SYSROM_BB01R4_OS,
                                             SYSROM_BB00R1, SYSROM_AA01R11, SYSROM_AA00R10,
                                             SYSROM_BB01R59, SYSROM_BB01R59A, SYSROM_BB02R3,
                                             SYSROM_BB02R3V4, SYSROM_CC01R4, SYSROM_XL_CUSTOM, -1};
static int const autochoose_order_xe[] = {SYSROM_BB01R3, SYSROM_BB01R4_OS, SYSROM_BB01R2,
                                          SYSROM_BB00R1, SYSROM_AA01R11, SYSROM_AA00R10,
                                          SYSROM_BB01R59, SYSROM_BB01R59A, SYSROM_BB02R3,
                                          SYSROM_BB02R3V4, SYSROM_CC01R4, SYSROM_XL_CUSTOM, -1};
static int const autochoose_order_xegs[] = {SYSROM_BB01R4_OS, SYSROM_BB01R3, SYSROM_BB01R2,
                                            SYSROM_BB00R1, SYSROM_AA01R11, SYSROM_AA00R10,
                                            SYSROM_BB01R59, SYSROM_BB01R59A, SYSROM_BB02R3,
                                            SYSROM_BB02R3V4, SYSROM_CC01R4, SYSROM_XL_CUSTOM, -1};
static int const autochoose_order_5200[] = {SYSROM_5200, SYSROM_5200A, SYSROM_5200_CUSTOM, -1};
static int const autochoose_order_basic[] = {SYSROM_BASIC_C, SYSROM_BASIC_B, SYSROM_BASIC_A,
                                             SYSROM_BASIC_CUSTOM, -1};
...
void SYSROM_ChooseROMs(int machine_type, int ram_size, int tv_system, int *os_version,
                       int *basic_version, int *xegame_version) {
    int os_ver;
...
int SYSROM_ReadConfig(char *string, char *ptr) {
    ...
    if (id >= 0) {
        ...
        if (!MatchROMVersionParameter(ptr, autochoose_order_800_ntsc,
                                      &SYSROM_os_versions[Atari800_MACHINE_800]))
            return FALSE;
    }
    else if (strcmp(string, "OS_XL/XE_VERSION") == 0) {
        if (!MatchROMVersionParameter(ptr, autochoose_order_600xl,
                                      &SYSROM_os_versions[Atari800_MACHINE_XLXE]))
            return FALSE;
    }
    else if (strcmp(string, "OS_5200_VERSION") == 0) {
        if (!MatchROMVersionParameter(ptr, autochoose_order_5200,
        ...
        if (SYSROM_SetPath(ptr, 12, SYSROM_AA00R10, SYSROM_AA01R11, SYSROM_BB00R1, SYSROM_BB01R2,
                           SYSROM_BB02R3, SYSROM_BB02R3V4, SYSROM_CC01R4, SYSROM_BB01R3,
                           SYSROM_BB01R4_OS, SYSROM_BB01R59, SYSROM_BB01R59A, SYSROM_XL_CUSTOM) !=
            SYSROM_OK)
            return FALSE;
    }
        ...
        if (SYSROM_SetPath(ptr, 4, SYSROM_BASIC_A, SYSROM_BASIC_B, SYSROM_BASIC_C,
                           SYSROM_BASIC_CUSTOM) != SYSROM_OK)
            return FALSE;
    }
    else return FALSE;
    return TRUE;
...
int SYSROM_Initialise(int *argc, char *argv[]) {
    ...
    for (i = j = 1; i < *argc; i++) {
        ...
        if (strcmp(argv[i], "-osa_rom") == 0) {
            ...
            if (i_a) {
                if (SYSROM_SetPath(argv[++i], 12, SYSROM_AA00R10, SYSROM_AA01R11, SYSROM_BB00R1,
                                   SYSROM_BB01R2, SYSROM_BB02R3, SYSROM_BB02R3V4, SYSROM_CC01R4,
                                   SYSROM_BB01R3, SYSROM_BB01R4_OS, SYSROM_BB01R59, SYSROM_BB01R59A,
                                   SYSROM_XL_CUSTOM) != SYSROM_OK)
                    a_i = TRUE;
            }
            else a_m = TRUE;
        }
            ...
            if (i_a) {
                if (SYSROM_SetPath(argv[++i], 3, SYSROM_5200, SYSROM_5200A, SYSROM_5200_CUSTOM) !=
                    SYSROM_OK)
                    a_i = TRUE;
            }
            else a_m = TRUE;
            ...
            if (i_a) {
                if (SYSROM_SetPath(argv[++i], 4, SYSROM_BASIC_A, SYSROM_BASIC_B, SYSROM_BASIC_C,
                                   SYSROM_BASIC_CUSTOM) != SYSROM_OK)
                    a_i = TRUE;
            }
            else a_m = TRUE;
            ...
            if (i_a) {
                if (!MatchROMVersionParameter(argv[++i], autochoose_order_800_ntsc,
                                              &SYSROM_os_versions[Atari800_MACHINE_800]))
                    a_i = TRUE;
            }
            else a_m = TRUE;
            ...
            if (i_a) {
                if (!MatchROMVersionParameter(argv[++i], autochoose_order_600xl,
                                              &SYSROM_os_versions[Atari800_MACHINE_XLXE]))
                    a_i = TRUE;
            }
            else a_m = TRUE;
            ...
            if (i_a) {
                if (!MatchROMVersionParameter(argv[++i], autochoose_order_5200,
                                              &SYSROM_os_versions[Atari800_MACHINE_5200]))
                    a_i = TRUE;
            }
            else a_m = TRUE;
            ...
            if (i_a) {
                if (!MatchROMVersionParameter(argv[++i], autochoose_order_basic,
            ...
            if (i_a) {
                if (!MatchROMVersionParameter(argv[++i], autochoose_order_xegame,
                                              &SYSROM_xegame_version))
                    a_i = TRUE;
            }
            else a_m = TRUE;
        }
