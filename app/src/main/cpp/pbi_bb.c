...
static int bb_rom_high_bit = 0x00;
/*0x10*/
static char bb_rom_filename[FILENAME_MAX];
static UBYTE bb_PCR = 0;
...
UBYTE PBI_BB_D1GetByte(UWORD addr, int no_side_effects) {
    ...
    if (addr == 0xd1be) result = 0xff;
        ...
        /* status */
        result = ((!(PBI_SCSI_REQ)) << 7) | ((!PBI_SCSI_BSY) << 6) | ((!PBI_SCSI_SEL) << 2) |
                 ((!PBI_SCSI_CD) << 1) | (!PBI_SCSI_IO);
    }
    else if (addr == 0xd171) {
        if (bb_scsi_enabled) {
    ...
    D(if (addr != 0xd1ff) printf("BB Read:%4x  PC:%4x byte=%2x\n", addr,
                                 CPU_remember_PC[(CPU_remember_PC_curpos - 1) %
                                                 CPU_REMEMBER_PC_STEPS], result));
    return result;
}
    ...
    if (addr == 0xd170) {
        ...
        if (bb_rom_high_bit != ((byte & 0x04) << 2) && bb_rom_size == 0x10000) {
            ...
            if (bb_rom_bank > 0 && bb_rom_bank < 8) {
                memcpy(MEMORY_mem + 0xd800, bb_rom + (bb_rom_bank + bb_rom_high_bit) * 0x800,
                       0x800);
