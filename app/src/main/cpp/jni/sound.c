...
static int OSL_init(void) {
    ...
    SLDataFormat_PCM dfpcm = {
            ...
            .samplesPerSec    = (snd_mixrate == 44100) ? SL_SAMPLINGRATE_44_1 :
                                ...
                                0,
            .bitsPerSample    = (at_sixteenbit) ? SL_PCMSAMPLEFORMAT_FIXED_16
                                                : SL_PCMSAMPLEFORMAT_FIXED_8,
            .containerSize    = (at_sixteenbit) ? SL_PCMSAMPLEFORMAT_FIXED_16
                                                : SL_PCMSAMPLEFORMAT_FIXED_8,
    ...
    CHECK_OSL((*osl_engine)->GetInterface(osl_engine, *osl_SL_IID_ENGINE, &osl_engineif),
              "get engine i/f");

    /* mixer */
    CHECK_OSL((*osl_engineif)->CreateOutputMix(osl_engineif, &osl_mixer, 1, mixids, mixreq),
              "create mixer");
    CHECK_OSL((*osl_mixer)->Realize(osl_mixer, SL_BOOLEAN_FALSE), "realize mixer");
    dlmix.outputMix = osl_mixer;

    /* player */
    CHECK_OSL((*osl_engineif)->CreateAudioPlayer(osl_engineif, &osl_player, &dsrc, &dsnk, 1, apids,
                                                 apreq), "create player");
    CHECK_OSL((*osl_player)->Realize(osl_player, SL_BOOLEAN_FALSE), "realize player");
    CHECK_OSL((*osl_player)->GetInterface(osl_player, *osl_SL_IID_PLAY, &osl_playif),
              "get player i/f");
    CHECK_OSL((*osl_player)->GetInterface(osl_player, *osl_SL_IID_ANDROIDSIMPLEBUFFERQUEUE,
                                          &osl_bufqif), "get buffer queue i/f");
...
static int OSL_buf_alloc(void) {
    ...
    if (!(osl_soundbuf = (UBYTE *) malloc(osl_bufnum * osl_bufszbytes))) {
        Log_print("ERROR: Cannot allocate sound buffer for %d buffers of %d bytes each", osl_bufnum,
                  osl_bufszbytes);
        return FALSE;
    }
...
static int OSL_start_playback(void) {
    ...
    for (i = 0; i < osl_bufnum; i++) {
        ...
        CHECK_OSL((*osl_bufqif)->Enqueue(osl_bufqif, osl_soundbufptr[i], osl_bufszbytes),
                  "enqueue init buffer");
    }
...
int Sound_Initialise(int *argc, char *argv[]) {
    ...
    if (
        ...
        Android_osl_sound = FALSE;
        Sound_Exit();
        Log_print("Using legacy EmuAudio");
    } else
