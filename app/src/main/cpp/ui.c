...
static void SystemSettings(void) {
    ...
    enum {
        MOSAIC_OTHER = 65
    }; /* must be a value that's illegal for MEMORY_mosaic_num_banks */
    ...
    enum {
        N_MACHINES = (int) (sizeof(machine) / sizeof(machine[0]))
    };
    ...
    for (; ;) {
        ...
        if (SYSROM_os_versions[Atari800_machine_type] == SYSROM_AUTO) {
            ...
            if (auto_os == -1)
                ...
                sprintf(default_os_label, "%s (auto)",
                        FindMenuItem(os_menu_arrays[Atari800_machine_type], auto_os)->item);
            ...
            menu_array[1].suffix = FindMenuItem(os_menu_arrays[Atari800_machine_type],
                                                SYSROM_os_versions[Atari800_machine_type])->item;
        ...
        if (Atari800_machine_type == Atari800_MACHINE_5200)
            ...
            if (SYSROM_basic_version == SYSROM_AUTO) {
                ...
                if (auto_basic == -1)
                    ...
                    sprintf(default_basic_label, "%s (auto)",
                            FindMenuItem(basic_menu_array, auto_basic)->item);
        ...
        if (Atari800_machine_type != Atari800_MACHINE_XLXE)
            ...
            if (SYSROM_xegame_version == SYSROM_AUTO) {
                ...
                if (auto_xegame == -1)
                    ...
                    sprintf(default_xegame_label, "%s (auto)",
                            FindMenuItem(xegame_menu_array, auto_xegame)->item);
        ...
        menu_array[13].suffix = Atari800_machine_type != Atari800_MACHINE_XLXE ? "N/A" :
                                Atari800_keyboard_detached ? "detached (XEGS)"
                                                           : "integrated/attached";

        /* Set label for the "XL/XE MapRAM" action. */
        menu_array[14].suffix = (Atari800_machine_type != Atari800_MACHINE_XLXE ||
                                 MEMORY_ram_size < 20)
                                ? "N/A"
        ...
        switch (option) {
            case 0:
                option2 = UI_driver->fSelect(NULL, UI_SELECT_POPUP, sys_id, machine_menu_array,
                                             NULL);
                if (option2 >= 0) {
            ...
            case 1: {
                ...
                if (!rom_available)
                    ...
                    option2 = UI_driver->fSelect(NULL, UI_SELECT_POPUP,
                                                 SYSROM_os_versions[Atari800_machine_type],
                                                 os_menu_arrays[Atari800_machine_type], NULL);
                    if (option2 >= 0) {
            ...
            case 3:
                if (Atari800_machine_type != Atari800_MACHINE_5200) {
                    ...
                    if (!rom_available)
                        ...
                        option2 = UI_driver->fSelect(NULL, UI_SELECT_POPUP, SYSROM_basic_version,
                                                     basic_menu_array, NULL);
                        if (option2 >= 0) {
            ...
            case 4:
                if (Atari800_machine_type == Atari800_MACHINE_XLXE) {
                    ...
                    option2 = UI_driver->fSelect(NULL, UI_SELECT_POPUP,
                                                 Atari800_builtin_game ? SYSROM_xegame_version : 0,
                                                 xegame_menu_array, NULL);
                    if (option2 >= 0) {
                        if (option2 > 0) {
            ...
            case 5: {
                ...
                option2 = UI_driver->fSelect(NULL, UI_SELECT_POPUP, MEMORY_ram_size, menu_ptr,
                                             NULL);
                if (option2 >= 0) {
            ...
            case 7:
                if (Atari800_machine_type == Atari800_MACHINE_800) {
                    if (MEMORY_mosaic_num_banks == 0 || MEMORY_mosaic_num_banks == 4 ||
                        MEMORY_mosaic_num_banks == 20 || MEMORY_mosaic_num_banks == 36)
                        option2 = MEMORY_mosaic_num_banks;
                    else
                        option2 = MOSAIC_OTHER;
                    option2 = UI_driver->fSelect(NULL, UI_SELECT_POPUP, option2,
                                                 mosaic_ram_menu_array, NULL);
                    if (option2 >= 0) {
            ...
            case 8:
                if (Atari800_machine_type == Atari800_MACHINE_800) {
                    option2 = UI_driver->fSelect(NULL, UI_SELECT_POPUP, MEMORY_axlon_num_banks,
                                                 axlon_ram_menu_array, NULL);
                    if (option2 >= 0) {
...
static void DiskManagement(void) {
    static char drive_array[8][5] = {" D1:", " D2:", " D3:", " D4:", " D5:", " D6:", " D7:",
                                     " D8:"};
    ...
    for (; ;) {
        ...
        switch (dsknum) {
            case 8:
                if (UI_driver->fGetSaveFilename(set_filename, UI_saved_files_dir,
                ...
                break;
            case 9:
                if (UI_driver->fGetLoadFilename(set_filename, UI_saved_files_dir,
                                                UI_n_saved_files_dir)) {
            ...
            case 10:
                ...
                break;
            case 11:
                if (UI_driver->fGetSaveFilename(disk_filename, UI_atari_files_dir,
                                                UI_n_atari_files_dir)) {
                ...
                break;
            case 12:
                if (UI_driver->fGetLoadFilename(disk_filename, UI_atari_files_dir,
                                                UI_n_atari_files_dir)) {
                    ...
                    switch (fgetc(fp)) {
                        case 0x1f:
                            fclose(fp);
                            if (UI_driver->fGetSaveFilename(uncompr_filename, UI_atari_files_dir,
                                                            UI_n_atari_files_dir)) {
                                ...
                                fclose(fp2);
                                UI_driver->fMessage(success ? "Conversion successful"
                                                            : "Cannot convert this file", 1);
                            }
                            break;
                        case 0xf9:
                        case 0xfa:
            ...
            default:
                ...
                switch (seltype) {
                    case UI_USER_SELECT: /* "Enter" */
                        if (SIO_drive_status[dsknum] != SIO_OFF &&
                            SIO_drive_status[dsknum] != SIO_NO_DISK)
                            strcpy(disk_filename, SIO_filename[dsknum]);
                        if (UI_driver->fGetLoadFilename(disk_filename, UI_atari_files_dir,
                                                        UI_n_atari_files_dir)) if (!SIO_Mount(
                                dsknum + 1, disk_filename, FALSE))
                            CantLoad(disk_filename);
                        break;
...
static void CartManagement(void) {
    ...
    for (; ;) {
        ...
        switch (option) {
            case 0:
                if (UI_driver->fGetLoadFilename(cart_filename, UI_atari_files_dir,
                                                UI_n_atari_files_dir)) {
                    FILE *f;
                    int nbytes;
                    int type;
                    ...
                    if (!UI_driver->fGetSaveFilename(cart_filename, UI_atari_files_dir,
                                                     UI_n_atari_files_dir))
                        break;
            ...
            case 1:
                if (UI_driver->fGetLoadFilename(cart_filename, UI_atari_files_dir,
                    ...
                    if (nbytes <= 0 || fread(&header, 1, sizeof(header), f) != sizeof(header)
                        || header.id[0] != 'C' || header.id[1] != 'A' || header.id[2] != 'R' ||
                        header.id[3] != 'T') {
                        fclose(f);
            ...
            case 2:
                switch (seltype) {
                    case UI_USER_SELECT: /* Enter */
                        if (UI_driver->fGetLoadFilename(CARTRIDGE_main.filename, UI_atari_files_dir,
                                                        UI_n_atari_files_dir)) {
                            ...
                            switch (r) {
                                ...
                                default:
                                    /* r > 0 */
                                    CARTRIDGE_SetTypeAutoReboot(&CARTRIDGE_main,
                                                                UI_SelectCartType(r));
                                    break;
                            }
                        }
            ...
            case 3:
                switch (seltype) {
                    case UI_USER_SELECT: /* Enter */
                        if (UI_driver->fGetLoadFilename(CARTRIDGE_piggyback.filename,
                                                        UI_atari_files_dir, UI_n_atari_files_dir)) {
    ...
    for (; ;) {
        ...
        if (CASSETTE_status == CASSETTE_STATUS_NONE)
            ...
            if (position > size)
                ...
                snprintf(position_string, sizeof(position_string) - 1, "%u/%u blocks", position,
                         size);
        }
        ...
        switch (option) {
            case 0:
                switch (seltype) {
                    case UI_USER_SELECT: /* Enter */
                        if (UI_driver->fGetLoadFilename(CASSETTE_filename, UI_atari_files_dir,
                                                        UI_n_atari_files_dir)) {
...
static void AdvancedHOptions(void) {
    ...
    for (; ;) {
        ...
        menu_array[1].suffix = (i > 0) ? ((i == 1) ? "Backspace: close" : "Backspace: close all")
                                       : NULL;
        for (i = 0; i < 4; i++)
            menu_array[3 + i].suffix =
                    Devices_h_current_dir[i][0] != '\0' ? "Backspace: reset to root" : NULL;
...
static void ConfigureDirectories(void) {
    ...
    for (; ;) {
        ...
        for (i = 0; i < 8; i++) {
            menu_array[1 + i].flags = (i < UI_n_atari_files_dir) ? (UI_ITEM_FILESEL | UI_ITEM_TIP)
                                                                 : UI_ITEM_HIDDEN;
            menu_array[11 + i].flags = (i < UI_n_saved_files_dir) ? (UI_ITEM_FILESEL | UI_ITEM_TIP)
                                                                  : UI_ITEM_HIDDEN;
        ...
        if (flags != 0) {
            switch (seltype) {
                ...
                case UI_USER_DRAG_DOWN:
                    if (option != UI_n_atari_files_dir - 1 &&
                        option != 10 + UI_n_saved_files_dir - 1) {
                        strcpy(tmp_dir, menu_array[1 + option].item);
...
static void AtariSettings(void) {
    ...
    for (; ;) {
        ...
        FindMenuItem(menu_array, 4)->suffix = Devices_enable_h_patch ? (Devices_h_read_only
                                                                        ? "Read-only"
                                                                        : "Read/write") : "No ";
        ...
        switch (option) {
            ...
            case 12:
                ...
                if (UI_driver->fEditString("Print command", tmp_command,
                                           sizeof(tmp_command))) if (!Devices_SetPrintCommand(
                        tmp_command))
...
#ifndef DREAMCAST
            case 15:
                UI_driver->fMessage(CFG_WriteConfig() ? "Configuration file updated"
                                                      : "Error writing configuration file", 1);
                break;
            case 16:
...
static int ColourSettingToSlider(int index) {
    /* min <= value <= max */
    return (int) Util_round(
            (*(colour_controls[index].setting) - colour_controls[index].min) * 100.0 /
...
static double SliderToColourSetting(int value, int index) {
    /* 0 <= value <= 100 */
    return (double) value * (colour_controls[index].max - colour_controls[index].min) / 100.0 +
           colour_controls[index].min;
}
...
static void DisplaySettings(void) {
    ...
    static char const *const colours_preset_names[] = {"Standard", "Deep black", "Vibrant",
                                                       "Custom"};
#endif
    ...
    for (; ;) {
        ...
        switch (option) {
            ...
            artif_menu_array[ARTIFACT_PAL_BLEND].flags =
                Atari800_tv_mode == Atari800_TV_PAL ? UI_ITEM_ACTION : UI_ITEM_HIDDEN;
#endif /* PAL_BLENDING */
                option2 = UI_driver->fSelect(NULL, UI_SELECT_POPUP, ARTIFACT_mode, artif_menu_array,
                                             NULL);
                if (option2 >= 0)
            ...
            case 11:
                ...
                if (ANTIC_artif_mode != 0) {
                    option2 = UI_driver->fSelect(NULL, UI_SELECT_POPUP, ANTIC_artif_mode - 1,
                                                 artif_mode_menu_array, NULL);
                    if (option2 >= 0) {
...
#if SUPPORTS_PLATFORM_PALETTEUPDATE
            case 12:
                option2 = UI_driver->fSelect(NULL, UI_SELECT_POPUP, colours_preset,
                                             colours_preset_menu_array, NULL);
                if (option2 >= 0) {
            ...
            case 21:
                switch (seltype) {
                    case UI_USER_SELECT:
                        if (UI_driver->fGetLoadFilename(Colours_external->filename,
                                                        UI_saved_files_dir, UI_n_saved_files_dir)) {
...
#else
            case 2:
                option2 = UI_driver->fSelect(NULL, UI_SELECT_POPUP, INPUT_mouse_mode,
                                             mouse_mode_menu_array, NULL);
                if (option2 >= 0)
...
#endif
        FindMenuItem(menu_array, 9)->suffix = POKEYSND_enable_new_pokey ? "N/A"
                                                                        : POKEYSND_bienias_fix
                                                                          ? "Yes" : "No ";
