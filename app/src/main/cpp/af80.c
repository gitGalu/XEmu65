...
static int rom_bank_select;
/* bits 0-3 of d5f7, $0-$f 16 banks */
static int not_rom_output_enable;
/* bit 4 of d5f7 0 = Enable ROM 1 = Disable ROM */
static int not_right_cartridge_rd4_control;
/* 0=$8000-$9fff cart ROM, 1= $8000-$9fff system RAM */
...
static int video_bank_select;
/* bits 0-3 of d5f6, $0-$f 16 banks */
    ...
    if (!not_enable_2k_character_ram) {
        ...
        if (video_bank_select == 0) {
            ...
            D(printf("AF80 Read addr:%4x cpu:%4x\n", addr,
                     CPU_remember_PC[(CPU_remember_PC_curpos - 1) % CPU_REMEMBER_PC_STEPS]));
        }
    }
    return result;
}
...
void AF80_D6PutByte(UWORD addr, UBYTE byte) {
    if (!not_enable_2k_character_ram) {
        ...
        if (video_bank_select == 0) {
            ...
            D(if (1 || (addr != 0xd618 && addr != 0xd619))
                  printf("AF80 Write addr:%4x byte:%2x, cpu:%4x\n", addr, byte,
                         CPU_remember_PC[(CPU_remember_PC_curpos - 1) % CPU_REMEMBER_PC_STEPS]));
        }
        else {
...
#define AF80_CELL_HEIGHT 10
    ...
    if (row >= crtreg[0x10]) {
        screen_pos =
                (row - crtreg[0x10]) * 80 + column + crtreg[0x0e] + ((crtreg[0x0f] & 0x3f) << 8);
    }
    else {
