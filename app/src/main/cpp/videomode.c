...
typedef struct display_mode_t {
    unsigned int min_w;
    /* Minimum horizontal display resolution to show this mode */
    unsigned int min_h;
    /* Minimum vertical display resolution to show this mode */
    unsigned int src_width;
    /* Horizontal resolution of source image in this mode */
    unsigned int src_height;
    /* Vertical resolution of source image in this mode */
    ...
    /* Pixel aspect ratio of this mode */
    unsigned int (*src2out_w_func)(unsigned int);

    /* Function that converts source image width to output image width */
    unsigned int (*out2src_w_func)(
            unsigned int); /* Function that converts output image width to source image width */
} display_mode_t;
...
static void ComputeVideoArea(VIDEOMODE_resolution_t const *res,
                             VIDEOMODE_resolution_t const *screen_res,
    ...
    if (VIDEOMODE_keep_aspect == VIDEOMODE_KEEP_ASPECT_REAL) {
        if (rotate)
            asp_ratio = display_modes[display_mode].asp_ratio * screen_res->height /
                        VIDEOMODE_host_aspect_ratio_h
                        * VIDEOMODE_host_aspect_ratio_w / screen_res->width;
        else
            asp_ratio = display_modes[display_mode].asp_ratio * screen_res->width /
                        VIDEOMODE_host_aspect_ratio_w
...
static void CropVideoArea(VIDEOMODE_resolution_t const *screen_res, unsigned int *out_w,
                          unsigned int *out_h, double mult_w, double mult_h) {
...
static void SetVideoMode(VIDEOMODE_resolution_t *res, VIDEOMODE_MODE_t display_mode,
        ...
        VIDEOMODE_vertical_offset = VIDEOMODE_MAX_VERTICAL_AREA;
    {
        ...
        else if (x > (int) (display_modes[display_mode].src_width - VIDEOMODE_src_offset_left -
                            VIDEOMODE_src_width))
            x = (int) display_modes[display_mode].src_width - VIDEOMODE_src_offset_left -
                VIDEOMODE_src_width;
        if (y < -(int) VIDEOMODE_src_offset_top)
            y = -(int) VIDEOMODE_src_offset_top;
        else if (y > (int) (display_modes[display_mode].src_height - VIDEOMODE_src_offset_top -
                            VIDEOMODE_src_height))
            y = (int) display_modes[display_mode].src_height - VIDEOMODE_src_offset_top -
                VIDEOMODE_src_height;
...
int VIDEOMODE_SetCustomHorizontalArea(unsigned int value) {
    ...
    if (value != VIDEOMODE_custom_horizontal_area ||
...
int VIDEOMODE_SetCustomVerticalArea(unsigned int value) {
    ...
    if (value != VIDEOMODE_custom_vertical_area ||
...
static void UpdateTvSystemSettings(void) {
    display_modes[VIDEOMODE_MODE_NORMAL].asp_ratio =
            Atari800_tv_mode == Atari800_TV_PAL ? pixel_aspect_ratio_pal : pixel_aspect_ratio_ntsc;
}
...
int VIDEOMODE_ReadConfig(char *option, char *ptr) {
    if (strcmp(option, "VIDEOMODE_WINDOW_WIDTH") == 0)
        ...
        if (strcmp(ptr, "AUTO") == 0)
            ...
            return ParseAspectRatio(ptr, &VIDEOMODE_host_aspect_ratio_w,
                                    &VIDEOMODE_host_aspect_ratio_h);
    }
#if COLUMN_80
...
int VIDEOMODE_Initialise(int *argc, char *argv[]) {
    ...
    for (i = j = 1; i < *argc; i++) {
        ...
        if (strcmp(argv[i], "-win-width") == 0) {
            ...
            if (i_a) {
                int idx = CFG_MatchTextParameter(argv[++i], horizontal_area_cfg_strings,
            ...
            if (i_a) {
                int idx = CFG_MatchTextParameter(argv[++i], vertical_area_cfg_strings,
            ...
            if (i_a) {
                int idx = CFG_MatchTextParameter(argv[++i], stretch_cfg_strings,
            ...
            if (i_a) {
                if ((VIDEOMODE_fit = CFG_MatchTextParameter(argv[++i], fit_cfg_strings,
                                                            VIDEOMODE_FIT_SIZE)) < 0)
                    a_i = TRUE;
            }
            else a_m = TRUE;
        }
            ...
            if (i_a) {
                if ((VIDEOMODE_keep_aspect = CFG_MatchTextParameter(argv[++i],
                                                                    keep_aspect_cfg_strings,
            ...
            if (i_a) {
                if (strcmp(argv[++i], "auto") == 0)
                    ...
                    a_i = !ParseAspectRatio(argv[i], &VIDEOMODE_host_aspect_ratio_w,
                                            &VIDEOMODE_host_aspect_ratio_h);
            }
            else a_m = TRUE;
        }
            ...
            if (strcmp(argv[i], "-help") == 0) {
                ...
                Log_print(
                        "\t-horiz-shift <num>          Set horizontal shift of the visible area (-%i..%i)",
                        VIDEOMODE_MAX_HORIZONTAL_AREA, VIDEOMODE_MAX_HORIZONTAL_AREA);
                Log_print(
                        "\t-vert-shift <num>           Set vertical shift of the visible area (-%i..%i)",
                        VIDEOMODE_MAX_VERTICAL_AREA, VIDEOMODE_MAX_VERTICAL_AREA);
...
static int CompareResolutions(const void *res1, const void *res2) {
    unsigned int w = ((const VIDEOMODE_resolution_t *) res1)->width -
                     ((const VIDEOMODE_resolution_t *) res2)->width;
    if (w != 0)
        return w;
    return ((const VIDEOMODE_resolution_t *) res1)->height -
           ((const VIDEOMODE_resolution_t *) res2)->height;
}
