...
int SIO_Mount(int diskno, const char *filename, int b_open_readonly) {
    ...
    switch (header.magic1) {
            ...
            /* DCM */
        {
            ...
            Util_rewind(f);
            if (fread(&header, 1, sizeof(struct AFILE_ATR_Header), f) !=
        ...
        while (trackoffset > 0 && trackoffset < file_length) {
            ...
            if (tracktype == 0) {
                ...
                for (j = 0; j < sectorcnt; j++) {
                    ...
                    sector->sec_rot_pos[sector->sec_count] = (unsigned int) (percent_rot *
                                                                             VAPI_CYCLES_PER_ROT);
                    sector->sec_offset[sector->sec_count] =
                            VAPI_32(sectorheader.startdata) + trackoffset;
                ...
                Log_print("Unknown VAPI track type Track:%d Type:%d", trackheader.tracknum,
                          tracktype);
            }
            trackoffset += next;
        }
...
void SIO_SizeOfSector(UBYTE unit, int sector, int *sz, ULONG *ofs) {
    ...
    if (image_type[unit] == IMAGE_TYPE_PRO) {
        ...
        size = 128;
        offset = header_size +
                 (sector - 1) * (boot_sectors_type[unit] == BOOT_SECTORS_PHYSICAL ? 256 : 128);
    }
    else {
        size = sectorsize[unit];
        offset = header_size + (boot_sectors_type[unit] == BOOT_SECTORS_LOGICAL ? 0x180 : 0x300) +
                 (sector - 4) * size;
    }
    ...
    if (image_type[unit] == IMAGE_TYPE_PRO) {
        ...
        if (buffer[5] != 0) {
            ...
            if (dupnum != 0) {
                ...
                /* read sector header */
                if (fread(buffer, 1, 12, disk[unit]) < 12) {
                    Log_print("Error in header2 of .pro image: sector:%d dupnum:%d", sector,
                              dupnum);
                    return 'E';
                }
            }
...
int SIO_FormatDisk(int unit, UBYTE *buffer, int sectsize, int sectcount) {
    ...
    if (is_atr) {
        struct AFILE_ATR_Header header;
        ULONG disksize =
                (bootsectsize * bootsectcount + sectsize * (sectcount - bootsectcount)) >> 4;
...
int SIO_DriveStatus(int unit, UBYTE *buffer) {
    ...
    if (io_success[unit] != 0 && image_type[unit] == IMAGE_TYPE_PRO) {
        ...
        Log_print("Drive Status unit %d %x %x %x %x", unit, buffer[0], buffer[1], buffer[2],
                  buffer[3]);
        return 'C';
    }
...
void SIO_Handler(void) {
    ...
    if (MEMORY_dGetByte(0x300) != 0x60 && unit < SIO_MAX_DRIVES &&
        (SIO_drive_status[unit] != SIO_OFF || BINLOAD_start_binloading)) {    /* UBYTE range ! */
#ifdef DEBUG
        ...
        switch (cmd) {
            ...
            case 0xA1:                /* xf551 hispeed */
                ...
                if (realsize == length) {
                    result = SIO_FormatDisk(unit, DataBuffer, realsize,
                                            SIO_format_sectorcount[unit]);
                    if (result == 'C')
                        MEMORY_CopyToMem(DataBuffer, data, realsize);
                }
                else {
...
#endif
            realsize = SIO_format_sectorsize[unit];
            DataBuffer[0] = SIO_FormatDisk(unit, DataBuffer + 1, realsize,
                                           SIO_format_sectorcount[unit]);
            DataBuffer[1 + realsize] = SIO_ChkSum(DataBuffer + 1, realsize);
            DataIndex = 0;
            ExpectedBytes = 2 + realsize;
            ...
            if (CommandIndex < ExpectedBytes) {
                ...
                if (CommandIndex >= ExpectedBytes) {
                    if (CommandFrame[0] >= 0x31 && CommandFrame[0] <= 0x38 &&
                        (SIO_drive_status[CommandFrame[0] - 0x31] != SIO_OFF ||
...
int SIO_RotateDisks(void) {
    ...
    for (i = 1; i < SIO_MAX_DRIVES; i++) {
        if (strcmp(tmp_filenames[i], "None") && strcmp(tmp_filenames[i], "Off") &&
            strcmp(tmp_filenames[i], "Empty")) {
            if (!SIO_Mount(i, tmp_filenames[i],
                           FALSE)) /* Note that this is NOT i-1 because SIO_Mount is 1 indexed */
                bSuccess = FALSE;
        }
    }

    i = SIO_MAX_DRIVES - 1;
    while (i > -1 && (!strcmp(tmp_filenames[i], "None") || !strcmp(tmp_filenames[i], "Off") ||
                      !strcmp(tmp_filenames[i], "Empty"))) {
        i--;
    }
