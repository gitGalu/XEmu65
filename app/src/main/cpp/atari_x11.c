...
static int keypad_mode = -1;
/* Joystick */
static int keypad_trig = 1;
/* Keypad Trigger Position */
static int keypad_stick = 0x0f;
/* Keypad Joystick Position */

static int xmouse_mode = -1;
/* Joystick, Paddle and Light Pen */
static int mouse_stick;
...
int PLATFORM_Initialise(int *argc, char *argv[]) {
    ...
    for (i = j = 1; i < *argc; i++) {
        ...
        if (strcmp(argv[i], "-small") == 0)
            ...
            windowsize = Huge;
        else if (strcmp(argv[i], "-clip_x") == 0) if (i_a)
            clipping_x = atoi(argv[++i]);
        else a_m = TRUE;
        else if (strcmp(argv[i], "-clip_y") == 0) if (i_a)
            clipping_y = atoi(argv[++i]);
        else a_m = TRUE;
        else if (strcmp(argv[i], "-clip_width") == 0) if (i_a)
            clipping_width = atoi(argv[++i]);
        else a_m = TRUE;
        else if (strcmp(argv[i], "-clip_height") == 0) if (i_a)
 ...
 */
    ...
    xswda.event_mask = KeyPressMask | KeyReleaseMask | ExposureMask | VisibilityChangeMask |
                       FocusChangeMask /* mmm */;
    xswda.colormap = cmap;
