...
/* Filter */
static int pokey_frq;
/* Hz - for easier resampling */
...
typedef struct stPokeyState {
    ...
    /* AUDF0 recalculated */
    int c0divstart_p;
    /* start value when c1_f0 */
    int c0diva;
    /* AUDF0 register */

    int c0t1;
    /* D - 5bit, Q goes to sw3 */
    int c0t2;
    /* D - out sw2, Q goes to sw4 and t3 */
    int c0t3;
    /* D - out t2, q goes to xor */

    int c0sw1;
    /* in1 - 4bit, in2 - 17bit, out goes to sw2 */
    int c0sw2;
    /* in1 - /Q t2, in2 - out sw1, out goes to t2 */
    int c0sw3;
    /* in1 - +5, in2 - Q t1, out goes to C t2 */
    int c0sw4;
    /* hi-pass sw */
...
static int remez_filter_table(double resamp_rate, /* output_rate/input_rate */
    ...
    *cutoff = 0.95 * 0.5 * resamp_rate;
    ...
    REMEZ_CreateFilter(filter_data, (size / interlevel) + 1, 2, bands, desired, weights,
                       REMEZ_BANDPASS);
...
#ifdef VOL_ONLY_SOUND
        buffer[0] = (UBYTE) floor(
                (generate_sample(pokey_states) + POKEYSND_sampout - MAX_SAMPLE / 2.0)
                * (255.0 / MAX_SAMPLE / 4 * M_PI * 0.95) + 128 + 0.5 + 0.5 * rand() / RAND_MAX -
                0.25);
#else
