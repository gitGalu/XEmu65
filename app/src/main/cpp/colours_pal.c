...
static void GetYUVFromGenerated(double yuv_table[256 * 5]) {
    ...
    /* Phase delays of colorbursts in even and odd lines. They are equal to
       Hue 1. */
    double const even_burst_del =
            base_del + add_del * del_coeffs.even[0].add + del_adj * del_coeffs.even[0].mult;
    double const odd_burst_del =
            base_del + add_del * del_coeffs.odd[0].add + del_adj * del_coeffs.odd[0].mult;
...
int COLOURS_PAL_Initialise(int *argc, char *argv[]) {
    ...
    for (i = j = 1; i < *argc; i++) {
        ...
        if (strcmp(argv[i], "-pal-saturation") == 0) {
            ...
            if (i_a) {
                Util_strlcpy(COLOURS_PAL_external.filename, argv[++i],
                             sizeof(COLOURS_PAL_external.filename));
                /* Use the "loaded" flag to indicate that the palette must be loaded later. */
