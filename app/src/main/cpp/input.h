...
extern int INPUT_joy_autofire[4];
/* autofire mode for each Atari port */

extern int INPUT_joy_block_opposite_directions;
/* can't move joystick left
...
/* device emulated with mouse */
extern int INPUT_mouse_port;
/* Atari port, to which the emulated device is attached */
extern int INPUT_mouse_delta_x;
/* x motion since last frame */
extern int INPUT_mouse_delta_y;
/* y motion since last frame */
extern int INPUT_mouse_buttons;
/* buttons pressed (b0: left, b1: right, b2: middle */
...
/* max. value of POKEY's POT register */
extern int INPUT_mouse_pen_ofs_h;
/* light pen/gun horizontal offset (for calibration) */
extern int INPUT_mouse_pen_ofs_v;
/* light pen/gun vertical offset (for calibration) */
extern int INPUT_mouse_joy_inertia;
/* how long the mouse pointer can move (time in Atari frames)
                               after a fast motion of mouse */
extern int INPUT_direct_mouse;
/* When true, convert the mouse pointer
