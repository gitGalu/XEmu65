...
enum {
    DEFAULT_BUFFER_SIZE = 132
};

/* Baudrate for all written blocks and for reading from raw files. */
enum {
    DEFAULT_BAUDRATE = 600
};

struct IMG_TAPE_t {
    FILE *file;
    /* Stream for reading/writing of the tape image */
    int isCAS; /* Indicates if the file is in CAS format, or a raw binary file */
    UBYTE *buffer;
    /* Holds bytes of the last read or currently written data block */
    ...
    ULONG save_gap;
    /* Length of the IRG before the currently written block */
    int next_blockbyte;
    /* Index of the byte in this block that will be read next (counted from 0) */
    unsigned int current_block;
    /* Number of the currently-read/written block (counted from 0) */
    int block_is_fsk;
    /* FALSE - current chunk's type  is "data", otherwise "fsk " */
    int block_length;
    /* Length of the block currently held in BUFFER */
    int num_blocks; /* Number of data blocks in the whole file */
    ULONG block_offsets[MAX_BLOCKS];
    /* File offsets for each data block*/
    int block_baudrates[MAX_BLOCKS];
    /* Baudrates for each data block in the file */
    char description[CASSETTE_DESCRIPTION_MAX];
    /* Tape description, only for CAS files */
...
static int WriteRecord(IMG_TAPE_t *file) {
    ...
    file->num_blocks++;
    file->block_offsets[file->num_blocks] =
            file->block_offsets[file->num_blocks - 1] + file->block_length + 8;
...
int IMG_TAPE_Read(IMG_TAPE_t *file, unsigned int *duration, int *is_gap, UBYTE *byte) {
    ...
    if (file->next_blockbyte >= file->block_length) {
        ...
        if (gap > 0) {
            ...
            *duration =
                    gap * 1789 + gap * 790 / 1000; /* (gap * 1789790 / 1000), avoiding overflow */
            *is_gap = TRUE;
            return TRUE;
        }
    }
...
int IMG_TAPE_WriteByte(IMG_TAPE_t *file, UBYTE byte, unsigned int pokey_counter) {
    ...
    int put_delay =
            file->savetime / 1790 - 10 * pokey_counter / 895; /* better accuracy not needed */
    if (put_delay > 05) {
...
int IMG_TAPE_SerinStatus(IMG_TAPE_t *file, int event_time_left) {
    ...
    if (file->block_is_fsk) {
        ...
        if (event_time_left <
            ...
            bit = event_time_left /
                  (1789790 / (file->isCAS ? file->block_baudrates[file->current_block] : 600));
        }
        else {
            bit = 0;
        }
    ...
    while (ms > 0) {
        if (file->next_blockbyte < file->block_length) {
            if (file->block_is_fsk) {
                ...
                int bytes = ms * (file->isCAS ? file->block_baudrates[file->current_block] : 600) /
                            1000 / 10;
                if (bytes > file->block_length - file->next_blockbyte)
                    bytes = file->block_length - file->next_blockbyte;
                file->next_blockbyte += bytes;
                ms -= bytes * 10 * 1000 /
                      (file->isCAS ? file->block_baudrates[file->current_block] : 600);
            }
            continue;
        }
        else {
...
int IMG_TAPE_ReadToMemory(IMG_TAPE_t *file, UWORD dest_addr, int length) {
    ...
    MEMORY_CopyToMem(file->buffer + file->next_blockbyte, dest_addr,
                     read_length >= length ? length : read_length);
