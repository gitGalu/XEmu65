...
void CYCLE_MAP_Create(void) {
    ...
    int k = 0;
    antic_steal_map(1, 0, 0, 0, 0, 0, antic_cycles, &CYCLE_MAP_cpu2antic[k],
                    &CYCLE_MAP_antic2cpu[k]); /* blank line, or mode 8-F following line*/
    k = CYCLE_MAP_SIZE * (17 * 0 + 1);
    try_all_scroll(0, 1, 1, 0, &CYCLE_MAP_cpu2antic[k],
                   &CYCLE_MAP_antic2cpu[k]); /* mode 2,3,4,5 first line */
    k = CYCLE_MAP_SIZE * (17 * 1 + 1);
    try_all_scroll(0, 0, 1, 0, &CYCLE_MAP_cpu2antic[k],
                   &CYCLE_MAP_antic2cpu[k]); /* mode 2,3,4,5 following lines */
    k = CYCLE_MAP_SIZE * (17 * 2 + 1);
    try_all_scroll(1, 1, 1, 0, &CYCLE_MAP_cpu2antic[k],
                   &CYCLE_MAP_antic2cpu[k]); /* mode 6,7 first line */
    k = CYCLE_MAP_SIZE * (17 * 3 + 1);
    try_all_scroll(1, 0, 1, 0, &CYCLE_MAP_cpu2antic[k],
                   &CYCLE_MAP_antic2cpu[k]); /* mode 6,7 following lines */
    k = CYCLE_MAP_SIZE * (17 * 4 + 1);
    try_all_scroll(0, 0, 0, 1, &CYCLE_MAP_cpu2antic[k],
                   &CYCLE_MAP_antic2cpu[k]); /* mode 8,9 first line */
    k = CYCLE_MAP_SIZE * (17 * 5 + 1);
    try_all_scroll(1, 0, 0, 1, &CYCLE_MAP_cpu2antic[k],
                   &CYCLE_MAP_antic2cpu[k]); /* mode A,B,C  first line */
    k = CYCLE_MAP_SIZE * (17 * 6 + 1);
    try_all_scroll(2, 0, 0, 1, &CYCLE_MAP_cpu2antic[k],
                   &CYCLE_MAP_antic2cpu[k]); /* mode D,E,F  first line */
#ifdef TEST_CYCLE_MAP
    for(j = 0; j < 17 * 7 + 1; j++) {
...
static void try_all_scroll(int md, int use_char_index,
    ...
    antic_steal_map(width, md, scroll_offset, use_char_index, use_font,
                    use_bitmap, antic_cycles, &cpu2antic[CYCLE_MAP_SIZE * 0],
                    &antic2cpu[CYCLE_MAP_SIZE * 0]);
