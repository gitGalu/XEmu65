...
UBYTE POKEY_AUDCTL[POKEY_MAXPOKEYS];
/* AUDCTL (D208) */
...
void POKEY_PutByte(UWORD addr, UBYTE byte) {
    ...
    switch (addr) {
        ...
        case POKEY_OFFSET_AUDCTL:
            ...
            Update_Counter((1 << POKEY_CHAN1) | (1 << POKEY_CHAN2) | (1 << POKEY_CHAN3) |
                           (1 << POKEY_CHAN4));
        ...
        case POKEY_OFFSET_AUDF1:
            ...
            Update_Counter(
                    (POKEY_AUDCTL[0] & POKEY_CH1_CH2) ? ((1 << POKEY_CHAN2) | (1 << POKEY_CHAN1))
                                                      : (1 << POKEY_CHAN1));
        ...
        case POKEY_OFFSET_AUDF3:
            ...
            Update_Counter(
                    (POKEY_AUDCTL[0] & POKEY_CH3_CH4) ? ((1 << POKEY_CHAN4) | (1 << POKEY_CHAN3))
                                                      : (1 << POKEY_CHAN3));
        ...
        case POKEY_OFFSET_SEROUT:
            ...
            if ((POKEY_SKCTL & 0x08) == 0x00) {
                ...
                POKEY_DELAYED_SEROUT_IRQ = 312 * 50 * 10 * (POKEY_AUDF[POKEY_CHAN3] +
                                                            POKEY_AUDF[POKEY_CHAN4] * 0x100) /
                                           895000;
                /* safety check */
...
void POKEY_Frame(void) {
    random_scanline_counter %= (POKEY_AUDCTL[0] & POKEY_POLY9) ? POKEY_POLY9_SIZE
                                                               : POKEY_POLY17_SIZE;
}
...
void POKEY_Scanline(void) {
    ...
    if (POKEY_DELAYED_XMTDONE_IRQ > 0) if (--POKEY_DELAYED_XMTDONE_IRQ == 0) {
        POKEY_IRQST &= 0xf7;
        if (POKEY_IRQEN & 0x08) {
#ifdef DEBUG2
        ...
        if (POKEY_AUDCTL[0] & POKEY_CH1_CH2) {
            if (POKEY_AUDCTL[0] & POKEY_CH1_179)
                POKEY_DivNMax[POKEY_CHAN2] =
                        POKEY_AUDF[POKEY_CHAN2] * 256 + POKEY_AUDF[POKEY_CHAN1] + 7;
            else
                POKEY_DivNMax[POKEY_CHAN2] =
                        (POKEY_AUDF[POKEY_CHAN2] * 256 + POKEY_AUDF[POKEY_CHAN1] + 1) *
                        POKEY_Base_mult[0];
        }
        else
        ...
        if (POKEY_AUDCTL[0] & POKEY_CH3_CH4) {
            if (POKEY_AUDCTL[0] & POKEY_CH3_179)
                POKEY_DivNMax[POKEY_CHAN4] =
                        POKEY_AUDF[POKEY_CHAN4] * 256 + POKEY_AUDF[POKEY_CHAN3] + 7;
            else
                POKEY_DivNMax[POKEY_CHAN4] =
                        (POKEY_AUDF[POKEY_CHAN4] * 256 + POKEY_AUDF[POKEY_CHAN3] + 1) *
                        POKEY_Base_mult[0];
        }
        else
