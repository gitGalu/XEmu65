...
const unsigned char UI_BASIC_key_to_ascii[256] =
        {
                ,
...
static int Select(int default_item, int nitems, const char *item[],
    ...
    for (; ;) {
        ...
        ClearRectangle(0x94, xoffset, yoffset, xoffset + ncolumns * (itemwidth + 1) - 2,
                       yoffset + nrows - 1);
        col = 0;
        row = 0;
...
static int BasicUISelect(const char *title, int flags, int default_item, const UI_tMenuItem *menu,
                         int *seltype) {
    int nitems;
    int index;
...
static int BasicUISelectInt(int default_value, int min_value, int max_value) {
    ...
    Box(0x9a, 0x94, x1, y1, x2, y2);
    value = Select(
            (default_value >= min_value && default_value <= max_value) ? default_value - min_value
                                                                       : 0,
...
static void FilenamesAdd(const char *filename) {
    if (n_filenames >= FILENAMES_INITIAL_SIZE && (n_filenames & (n_filenames - 1)) == 0) {
        ...
        filenames = (const char **) Util_realloc((void *) filenames,
                                                 2 * n_filenames * sizeof(const char *));
    }
...
static int FileSelector(char *path, int select_dir, char pDirectories[][FILENAME_MAX],
                        int nDirectories) {
...
static int EditFilename(const char *title, char *filename, char directories[][FILENAME_MAX],
                        int n_directories) {
    ...
    if (!EditString(0x9a, 0x94, title, edited_filename, FILENAME_MAX, 1, 11, 36, directories,
                    n_directories))
        return FALSE;
...
static int BasicUIGetSaveFilename(char *filename, char directories[][FILENAME_MAX],
                                  int n_directories) {
#ifdef DO_DIR
    return EditFilename("Save as ([Tab] = directory locator)", filename, directories,
                        n_directories);
#else
...
static int BasicUIGetLoadFilename(char *filename, char directories[][FILENAME_MAX],
                                  int n_directories) {
#ifdef DO_DIR
