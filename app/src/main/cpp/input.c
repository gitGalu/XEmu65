...
int INPUT_joy_autofire[4] = {INPUT_AUTOFIRE_OFF, INPUT_AUTOFIRE_OFF, INPUT_AUTOFIRE_OFF,
                             INPUT_AUTOFIRE_OFF};
...
/* min. value of POKEY's POT register */
...
static int joy_multijoy_no = 0;
/* number of selected joy */
...
void INPUT_Frame(void) {
    ...
    static UBYTE last_stick[4] = {INPUT_STICK_CENTRE, INPUT_STICK_CENTRE, INPUT_STICK_CENTRE,
                                  INPUT_STICK_CENTRE};
    ...
    */
...
#endif
    i = Atari800_machine_type == Atari800_MACHINE_5200 ? INPUT_key_shift : (INPUT_key_code ==
                                                                            AKEY_BREAK);
    if (i && !last_key_break) {
        if (POKEY_IRQEN & 0x80) {
            POKEY_IRQST &= ~0x80;
    ...
    if (Atari800_machine_type != Atari800_MACHINE_5200 &&
        (INPUT_key_code & ~0x17) == AKEY_SHFTCTRL) {
        INPUT_key_code = AKEY_NONE;
    }
    if (INPUT_key_code >= 0) {
...
#endif
        if ((INPUT_joy_autofire[i] == INPUT_AUTOFIRE_FIRE && !TRIG_input[i]) ||
            (INPUT_joy_autofire[i] == INPUT_AUTOFIRE_CONT))
    ...
    switch (INPUT_mouse_mode) {
        ...
        case INPUT_MOUSE_KOALA:
            if (!INPUT_direct_mouse) {
                if (INPUT_mouse_mode != INPUT_MOUSE_PAD ||
                ...
                if (INPUT_mouse_mode == INPUT_MOUSE_KOALA ||
                    Atari800_machine_type == Atari800_MACHINE_5200)
        ...
        case INPUT_MOUSE_TRAK:
...

            {
                ...
                if (INPUT_mouse_mode == INPUT_MOUSE_TRAK) {
                    ...
                    STICK[INPUT_mouse_port] = (INPUT_mouse_mode == INPUT_MOUSE_AMIGA
                                               ? mouse_amiga_codes : mouse_st_codes)
        ...
        case INPUT_MOUSE_JOY:
            if (Atari800_machine_type == Atari800_MACHINE_5200) {
                ...
                int val = INPUT_joy_5200_center +
                          ((INPUT_mouse_delta_x * 2 * INPUT_mouse_speed) >> MOUSE_SHIFT);
                ...
                val = INPUT_joy_5200_center +
                      ((INPUT_mouse_delta_y * 2 * INPUT_mouse_speed) >> MOUSE_SHIFT);
                if (val < INPUT_joy_5200_min)
                    val = INPUT_joy_5200_min;
                else if (val > INPUT_joy_5200_max)
                    val = INPUT_joy_5200_max;
...
void INPUT_Scanline(void) {
    if (--scanline_counter == 0) {
        ...
        if (INPUT_mouse_mode == INPUT_MOUSE_TRAK) {
            ...
            STICK[INPUT_mouse_port] = (INPUT_mouse_mode == INPUT_MOUSE_AMIGA ? mouse_amiga_codes
                                                                             : mouse_st_codes)
            [(mouse_y & 3) * 4 + (mouse_x & 3)];
        }
        PIA_PORT_input[0] = (STICK[1] << 4) | STICK[0];
