...
static int load_roms(void) {
    if (Atari800_machine_type != Atari800_MACHINE_5200 && emuos_mode == 2) {
        ...
        SYSROM_ChooseROMs(Atari800_machine_type, MEMORY_ram_size, Atari800_tv_mode,
                          &Atari800_os_version, &basic_ver, &xegame_ver);
        if (Atari800_os_version == -1
            || !Atari800_LoadImage(SYSROM_roms[Atari800_os_version].filename, MEMORY_os,
                                   SYSROM_roms[Atari800_os_version].size)) {
            /* Missing OS ROM. */
            Atari800_os_version = -1;
            ...
            MEMORY_have_basic = basic_ver != -1 &&
                                Atari800_LoadImage(SYSROM_roms[basic_ver].filename, MEMORY_basic,
                                                   SYSROM_roms[basic_ver].size);
            ...
            if (Atari800_builtin_game) {
                ...
                if (xegame_ver == -1
                    || !Atari800_LoadImage(SYSROM_roms[xegame_ver].filename, MEMORY_xegame,
                                           SYSROM_roms[xegame_ver].size))
        ...
        if (strcmp(argv[i], "-atari") == 0) {
            ...
            if (strcmp(argv[i], "-run") == 0) {
                ...
                if (i_a) {
                    ...
                    if ((total_ram - 48) % 4 != 0 || MEMORY_mosaic_num_banks >= 0x40 ||
                ...
                if (i_a) {
                    ...
                    if ((total_ram - 32) % 16 != 0 ||
                        (MEMORY_axlon_num_banks != 0 && MEMORY_axlon_num_banks != 8 &&
                         MEMORY_axlon_num_banks != 16 && MEMORY_axlon_num_banks != 32 &&
                         MEMORY_axlon_num_banks != 64 && MEMORY_axlon_num_banks != 128 &&
                         MEMORY_axlon_num_banks != 256)) {
...
#endif
                    Log_print(
                            "\t-autosave-config Automatically save configuration on emulator exit");
...
static void autoframeskip(double curtime, double lasttime) {
    ...
    if (curtime - afs_lasttime > 0.5) {
        afs_ataritime = ((double) (Atari800_nframes - afs_lastframe)) /
                        ((double) (Atari800_tv_mode == Atari800_TV_PAL ? Atari800_FPS_PAL
                                                                       : Atari800_FPS_NTSC));
...
void Atari800_Sync(void) {
    ...
    double deltatime =
            1.0 / ((Atari800_tv_mode == Atari800_TV_PAL) ? Atari800_FPS_PAL : Atari800_FPS_NTSC);
    double curtime;
