...
CARTRIDGE_image_t CARTRIDGE_main = {CARTRIDGE_NONE, 0, 0, NULL, ""};
/* Left/Right cartridge */
CARTRIDGE_image_t CARTRIDGE_piggyback = {CARTRIDGE_NONE, 0, 0, NULL,
                                         ""}; /* Pass through cartridge for SpartaDOSX */
...
static void set_bank_A0BF(int disable_mask, int bank_mask) {
    if (active_cart->state & disable_mask)
        ...
        MEMORY_CopyROM(0xa000, 0xbfff,
                       active_cart->image + (active_cart->state & bank_mask) * 0x2000);
    }
}
...
static void set_bank_SDX_128(void) {
    if (active_cart->state & 8)
        ...
        MEMORY_CopyROM(0xa000, 0xbfff,
                       active_cart->image +
                       ((active_cart->state & 7) + ((active_cart->state & 0x10) >> 1)) * 0x2000);
    }
}
    ...
    if (Atari800_machine_type == Atari800_MACHINE_5200) {
        ...
        switch (active_cart->type) {
            ...
            case CARTRIDGE_5200_EE_16:
                ...
                break;
            case CARTRIDGE_5200_40:
                MEMORY_CopyROM(0x4000, 0x4fff,
                               active_cart->image + (active_cart->state & 0x03) * 0x1000);
                MEMORY_CopyROM(0x5000, 0x5fff, active_cart->image + 0x4000 +
                                               ((active_cart->state & 0x0c) >> 2) * 0x1000);
        ...
        switch (active_cart->type) {
            ...
            case CARTRIDGE_BBSB_40:
                MEMORY_Cart809fEnable();
                MEMORY_CartA0bfEnable();
                MEMORY_CopyROM(0x8000, 0x8fff,
                               active_cart->image + (active_cart->state & 0x03) * 0x1000);
                MEMORY_CopyROM(0x9000, 0x9fff, active_cart->image + 0x4000 +
                                               ((active_cart->state & 0x0c) >> 2) * 0x1000);
                MEMORY_CopyROM(0xa000, 0xbfff, active_cart->image + 0x8000);
#ifndef PAGED_ATTRIB
            ...
            case CARTRIDGE_RIGHT_8:
                if (Atari800_machine_type == Atari800_MACHINE_800) {
                    ...
                    if (!Atari800_builtin_basic
                        && (!Atari800_disable_basic || BINLOAD_loading_basic) &&
                        MEMORY_have_basic) {
                        MEMORY_CartA0bfEnable();
                        MEMORY_CopyROM(0xa000, 0xbfff, MEMORY_basic);
                    }
...
UBYTE CARTRIDGE_GetByte(UWORD addr, int no_side_effects) {
    ...
    return GetByte(&CARTRIDGE_main, addr, no_side_effects) &
...
static void PreprocessCart(CARTRIDGE_image_t *cart) {
    ...
    struct cross_map_t {
        ...
        /* Mapping of address (+ bank select) lines */
...
int CARTRIDGE_Initialise(int *argc, char *argv[]) {
    ...
    for (i = j = 1; i < *argc; i++) {
        ...
        if (strcmp(argv[i], "-cart") == 0) {
            ...
            if (i_a) {
                Util_strlcpy(CARTRIDGE_piggyback.filename, argv[++i],
                             sizeof(CARTRIDGE_piggyback.filename));
            ...
            if (i_a) {
                ...
                if (CARTRIDGE_piggyback.type < 0 ||
                    CARTRIDGE_piggyback.type > CARTRIDGE_LAST_SUPPORTED)
                    a_i = TRUE;
                else
            ...
            if (strcmp(argv[i], "-help") == 0) {
                ...
                Log_print("\t-cart-type <num>     Set cartridge type (0..%i)",
                          CARTRIDGE_LAST_SUPPORTED);
                Log_print("\t-cart2 <file>        Install piggyback cartridge");
                Log_print("\t-cart2-type <num>    Set piggyback cartridge type (0..%i)",
                          CARTRIDGE_LAST_SUPPORTED);
