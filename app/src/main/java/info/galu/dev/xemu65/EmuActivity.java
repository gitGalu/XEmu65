...
import info.galu.dev.xemu65.prefs.EmuPrefsActivity;
...
public class EmuActivity extends AppCompatActivity implements DigitalJoyCallback {
    ...
    private void nativeInit() {
        if ("ORIGINAL".equals(preferences.getString(Codes.PREF_ROM, null))) {
            NativeSetROMPath(getApplicationContext().getFilesDir().getPath() + "/atariroms/");
        } else {
            NativeSetROMPath(getApplicationContext().getFilesDir().getPath());
        }

        NativePrefMachine(7, isNtscEnabled()); // default 64KB
    ...
    private void prefsActionPerformed() {
        Intent prefsIntent = new Intent(this, EmuPrefsActivity.class);
        Bundle bundle = AnimUtils.getActivityTransitionParams(this).toBundle();
        startActivity(prefsIntent, bundle);
    }
        ...
        switch (requestCode) {
            case Codes.REQUEST_FILE_BROWSER:
                if (resultCode == Codes.RESULT_FILE_BROWSER_OK) {
                    ...
                    NativePrefMachine(cfg.getMemConfig().getNum(), isNtscEnabled());
                    loadGame(filePath);
                    if (savePath != null) {
    ...
    private boolean isNtscEnabled() {
        if ("NTSC".equals(preferences.getString(Codes.PREF_REGION, null))) {
            return true;
        } else {
            return false;
        }
